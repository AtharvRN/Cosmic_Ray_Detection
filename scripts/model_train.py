...
class train:
    ...
    def __init__(self, image, mask, ignore=None, sky=None, aug_sky=(0, 0), aug_img=(1, 1), noise=False, saturation=1e5,
        ...
        if gpu:
            ...
            # config = get_config()
            # self.network = WrappedModel(VisionTransformer(config, img_size=256, num_classes=2, zero_head=False, vis=True))
            # num_params = sum(p.numel() for p in self.network.parameters() if p.requires_grad)
            # print("Number of Parameters: ", num_params)
        ...
        # self.optimizer = optim.SGD(self.network.parameters(), lr=0.01,momentum=0.9,weight_decay=1e-4)
        lr = 0.005
        # auto_lr_decay = False
        if auto_lr_decay:
        ...
        self.DiceLoss = DiceLoss(n_classes=1)
    ...
    def validate_mask(self):
        ...
        FPR = FP / (FP + TN)
        if self.verbose:
            print('[TPR=%.5f, FPR=%.5f] @threshold = 0.5' % (TPR, FPR))
        return (lmask)
    ...
    def train(self):
        ...
        if self.verbose:
            ...
            print('')
        train_loss = self.train_phase0(self.n_epochs_phase0)
    ...
    def train_phase0(self, epochs):
        ...
        for epoch in self.tqdm(range(epochs), disable=self.disable_tqdm):
            ...
            for t, dat in enumerate(self.TrainLoader):
                loss = self.optimize_network(dat)
                if t%50 == 0:
                    print(f'epoch : {epoch}, batch : {t}, current_loss : {loss}')
            ...
            if self.verbose:
                ...
                print('Current Loss = %.6f' % (self.validation_loss[-1]))
         
                print('Avg. loss = %.6f' % (sum(self.validation_loss)/len(self.validation_loss)))
                # print('Dice_Score = %.6f' % (self.validation_dice_score[-1]))

                print("------------Training Loss ---------")
                print('Current Loss = %.6f' % (training_loss[-1]))
                print('Avg. loss = %.6f' % (sum(training_loss)/len(training_loss)))
    ...
    def backward_network(self):
        ...
        loss = 0.5*self.BCELoss(self.pdt_mask * (1 - self.ignore), self.mask * (1 - self.ignore)) + 0.5*self.DiceLoss(self.pdt_mask * (1 - self.ignore), self.mask * (1 - self.ignore))
